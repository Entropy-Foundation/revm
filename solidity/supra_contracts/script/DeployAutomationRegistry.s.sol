...
contract DeployAutomationRegistry is Script {
    ...
    function run() public {
        ...
        // ---------------------------------------------------------------------
        // Pre-compute proxy addresses for all contracts
        // ---------------------------------------------------------------------
        ...
        address coreProxyAddr = computeCreateAddress(msg.sender, currentNonce + 1);
        address registryProxyAddr = computeCreateAddress(msg.sender, currentNonce + 3);
        address controllerProxyAddr = computeCreateAddress(msg.sender, currentNonce + 5);

        console.log("Pre-computed AutomationCore proxy address: ", coreProxyAddr);
        console.log("Pre-computed AutomationRegistry proxy address: ", registryProxyAddr);
        console.log("Pre-computed AutomationController proxy address: ", controllerProxyAddr);
        ...
        LibConfig.InitializeParams memory coreParams = LibConfig.InitializeParams({
            ...
            controller: controllerProxyAddr,
            registry: registryProxyAddr,
            owner: msg.sender
        });

        bytes memory coreInitData = abi.encodeCall(AutomationCore.initialize, (coreParams));
        ...
        bytes memory registryInitData = abi.encodeCall(
            ...
            (coreProxyAddr, controllerProxyAddr, msg.sender)
        );
        ...
        bytes memory controllerInitData = abi.encodeCall(
            ...
            (coreProxyAddr, registryProxyAddr, msg.sender, true, cycleDurationSecs)
        );
