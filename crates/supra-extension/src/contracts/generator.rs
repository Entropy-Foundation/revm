...
sol! {
    ...
    contract Diamond {
        constructor(address _owner, address _diamond_cut);
    }
    ...
    contract DiamondCutFacet {
        function diamondCut(FacetCut[] calldata _diamondCut, address _diamond_init, bytes calldata _diamond_init_calldata);
    }
}
...
impl GenesisTransactionGenerator {
    ...
    pub fn prepare_genesis_transactions(
        ...
        if full_set {
            ...
            let erc20supra_address = self
                .address
                .create(GenesisTransactionTags::Erc20Supra as u64);
    ...
    ///  2. Diamond, // The wrapper contract of all the facets, main entry point of automation registry API
    ///  3. DiamondLoupeFacet, // Facet providing API to query registered facets
    ///  4. OwnershipFacet, // Facet to manage ownership credential updates and checks
    ///  5. ConfigFacet, // Facet to manage automation registry configuration
    ///  6. RegistryFacet, // Facet providing API for task registration, cancellation and registry state query
    ...
    fn setup_automation_registry(
        &mut self,
        owner: Address,
        _erc20_supra_address: Address,
        registry_config: AutomationRegistryConfig,
    ) -> Result<BTreeMap<GenesisTransactionTags, GenesisTransaction>> {
        let _config = registry_config
            .v1()
            .ok_or_else(|| anyhow!("Unhandled configuration version"))?;
        // Pre-compute all deployment addresses
        // nonce+0: DiamondCutFacet (implementation for Diamond proxy)
        // nonce+1: Diamond (proxy)
        // nonce+2: DiamondLoupeFacet
        // nonce+3: OwnershipFacet
        // nonce+4: ConfigFacet
        // nonce+5: RegistryFacet
        // nonce+6: CoreFacet
        // nonce+7: DiamondInit

        let diamond_cut_facet_addr = self.address.create(self.nonce);
        let diamond_addr = self.address.create(self.nonce + 1);
        let diamond_loupe_facet_addr = self.address.create(self.nonce + 2);
        let ownership_facet_addr = self.address.create(self.nonce + 3);
        let config_facet_addr = self.address.create(self.nonce + 4);
        let registry_facet_addr = self.address.create(self.nonce + 5);
        let core_facet_addr = self.address.create(self.nonce + 6);
        let diamond_init_addr = self.address.create(self.nonce + 7);
        ...
        let diamond_constructor_data = Diamond::constructorCall {
            _owner: owner,
            _diamond_cut: diamond_cut_facet_addr,
        }
        .abi_encode();
    ...
    fn setup_supra_nova_contracts(
        ...
        // For future implementation of Supra Nova and Bridge contracts deployment
        Ok(contract_txns)
    }
