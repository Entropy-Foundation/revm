...
use primitives::{address, hex, Address, TxKind};
...
/// Retrieved from https://github.com/mds1/multicall3 (README, "New Deployments" section).
pub const MULTICALL3_CODE: &[u8] =
    include_bytes!("canonical_singletons_bytecode/multicall3.bin");
...
pub fn generate_createx_transaction() -> GenesisTransaction {
    GenesisTransaction::create(CREATEX_DEPLOYER, CREATEX_CODE.to_owned(), 0, CREATEX_ADDRESS)
}
...
mod tests {
    ...
    // Byte-length canaries: a cheap tripwire against accidental truncation/corruption of the
    // embedded init-code assets.
    #[test]
    fn create2_factory_code_has_expected_length() {
        assert_eq!(CREATE2_FACTORY_CODE.len(), 83);
    }

    #[test]
    fn multicall3_code_has_expected_length() {
        assert_eq!(MULTICALL3_CODE.len(), 3840);
    }

    #[test]
    fn singleton_factory_code_has_expected_length() {
        assert_eq!(SINGLETON_FACTORY_CODE.len(), 340);
    }

    #[test]
    fn createx_code_has_expected_length() {
        assert_eq!(CREATEX_CODE.len(), 12054);
    }

    #[test]
    fn erc1820_registry_code_has_expected_length() {
        assert_eq!(ERC1820_REGISTRY_CODE.len(), 2533);
    }
